...
with tab_ml:
    ...
    if modo_ml == "🔗 Buscar direto da plataforma (API)":
        ...
        if tem_ricapet or tem_thapets:
            ...
            with col_d1:
                data_ini = st.date_input("De", value=ini_default, key="ml_api_ini")
            with col_d2:
                    ...
                    with st.spinner("Buscando pedidos no Mercado Livre..."):
                        try:
                            ...
                            if tem_ricapet:
                                ...
                                ords_r = ml_api.fetch_orders(tok["access_token"], uid, from_str, to_str)
                                st.info(f"Ricapet: {len(ords_r)} pedido(s) encontrado(s)")
                                if ords_r:
                                    arquivos.append(("relatorio_736787693.xlsx",
                                                     ml_api.orders_to_excel_bytes(ords_r, "Ricapet")))
                                    total_pedidos += len(ords_r)

                            if tem_thapets:
                                tok = st.session_state["ml_token_thapets"]
                                uid = st.session_state["ml_userid_thapets"]
                                ords_t = ml_api.fetch_orders(tok["access_token"], uid, from_str, to_str)
                                st.info(f"Thapets: {len(ords_t)} pedido(s) encontrado(s)")
                                if ords_t:
                                    arquivos.append(("relatorio_1139210125.xlsx",
                                                     ml_api.orders_to_excel_bytes(ords_t, "Thapets")))
                                    total_pedidos += len(ords_t)
                            ...
                            if not arquivos:
                                ...
                                st.success(f"Consolidado! {total_pedidos} pedido(s) processado(s).")
                                st.download_button(
    ...
    # ------------------------------------------------------------------
    # MODO ARQUIVO (mantido como fallback)
    # ------------------------------------------------------------------
    else:
        col1, col2 = st.columns(2)
        with col1:
